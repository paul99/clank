...
void GLES2Implementation::UnmapBufferSubDataCHROMIUM(const void* mem) {
  ...
  // Flushing after unmap lets the service side start processing commands sooner, but on
  // android class hardware the thread contention is worse than the latency hit.
