...
std::string g_os_info_;
std::string g_mobile_;
std::string g_chrome_version_;
...
std::string BuildChromeVersionInfo() {
  std::string cr_ver;
#if defined(OS_ANDROID)
  base::StringAppendF(
      &cr_ver,
      "Chrome/%s ",
      g_chrome_version_.c_str()
  );
#endif
  return cr_ver;
}
...
std::string BuildUserAgentFromProduct(const std::string& product) {
...
#if !defined(OS_ANDROID)
  const char* c_str_product = product.c_str();
#else
  // TODO(clank) Hide Chrome for now
  std::string clank_product = "";
  clank_product += g_mobile_;
  const char* c_str_product = clank_product.c_str();
#endif
  ...
  base::StringAppendF(
      ...
      " (KHTML, like Gecko) %s%s Safari/%d.%d",
      kUserAgentPlatform,
      webkit_glue::BuildOSCpuInfo().c_str(),
      WEBKIT_VERSION_MAJOR,
      WEBKIT_VERSION_MINOR,
      webkit_glue::BuildChromeVersionInfo().c_str(),
      c_str_product,
      WEBKIT_VERSION_MAJOR,
      WEBKIT_VERSION_MINOR);
  return user_agent;
...
#if defined(OS_ANDROID)
void InitUserAgent(std::string os_info, std::string mobile,
    std::string chrome_version) {
  DCHECK(g_os_info_.empty() && g_mobile_.empty() && g_chrome_version_.empty());
  g_os_info_ = os_info;
  g_mobile_ = mobile;
  g_chrome_version_ = chrome_version;
}

std::string GetDesktopUserAgent() {
  // TODO(dfalcantara): This should ideally be generated programatically.
