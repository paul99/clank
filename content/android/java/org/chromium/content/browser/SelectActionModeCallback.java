...
class SelectActionModeCallback implements ActionMode.Callback {
    ...
    protected static final int[] ACTION_MODE_ATTRS = {
    ...
    private void createActionMenu(ActionMode mode, Menu menu) {
        ...
        if (!mEditable) {
            if (isShareHandlerAvailable()) {
                menu.add(Menu.NONE, ID_SHARE, Menu.NONE, ChromeView.getResourceId(
                        "R.string.actionbar_share")).
                    setIcon(ChromeView.getResourceId("R.drawable.ic_menu_share_holo_light")).
                    setShowAsAction(
                            MenuItem.SHOW_AS_ACTION_ALWAYS | MenuItem.SHOW_AS_ACTION_WITH_TEXT);
            }

            if (!mIncognito && isWebSearchAvailable()) {
                menu.add(Menu.NONE, ID_SEARCH, Menu.NONE, ChromeView.getResourceId(
                        "R.string.actionbar_web_search")).
                    setIcon(ChromeView.getResourceId("R.drawable.ic_menu_search_holo_light")).
                    setShowAsAction(
                            MenuItem.SHOW_AS_ACTION_ALWAYS | MenuItem.SHOW_AS_ACTION_WITH_TEXT);
            }
        }
                ...
                if (!TextUtils.isEmpty(selection)) {
                    ...
                    try {
                        Intent i = Intent.createChooser(send, getContext().getString(
                                ChromeView.getResourceId("R.string.actionbar_share")));
