...
class DeviceOrientation implements SensorEventListener {
    ...
    // These fields lazily initialized by getHandler().
     ...
     *
     * TODO(husky): Maybe allow multiple native listeners?
     *
    ...
    /**
     * We register our listener with a Handle of null, so this callback will
     * happen on the application's UI thread.
     *
     * TODO(husky): Remove the native thread from profile_impl.cc and use a
     * Dalvik thread instead, owned by this object.
     */
    ...
    void getOrientationUsingGetRotationMatrix() {
        ...
        // the rotations are applied about the same axes and in the same order
        // as required by the
        ...
        maybeGotOrientation(alpha, beta, gamma);
    }
    ...
    synchronized void maybeGotOrientation(double alpha, double beta, double gamma) {
        if (mNativePtr != 0) {
    ...
    /**
     * See chrome/browser/device_orientation/device_orientation_android.cc
     */
