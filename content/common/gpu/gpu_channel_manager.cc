...
void GpuChannelManager::SetNativeWindow(
    int32 view_or_surface_id, int32 renderer_id, ANativeWindow* native_window) {
  GpuChannelMap::const_iterator iter = gpu_channels_.find(renderer_id);
  int surface_id = native_window ?
  ...
  if (iter != gpu_channels_.end()) {
    ...
    LOG(WARNING) << "GpuChannelManager::SetNativeWindow can't find render "
        << renderer_id;
  }
}
#endif
