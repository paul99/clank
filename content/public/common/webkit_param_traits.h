...
struct ParamTraits<WebKit::WebTextInputType> {
  ...
  static bool Read(const Message* m, void** iter, param_type* p) {
    ...
    if (!m->ReadInt(iter, &type))
      return false;
    *p = static_cast<WebKit::WebTextInputType>(type);
    return true;
  }
  static void Log(const param_type& p, std::string* l) {
    const char* type;
    ...
    case WebKit::WebTextInputTypePassword:
      type = "Password";
      break;
    case WebKit::WebTextInputTypeSearch:
      type = "Search";
      break;
     case WebKit::WebTextInputTypeEmail:
      type = "Email";
      break;
     case WebKit::WebTextInputTypeNumber:
      type = "Number";
      break;
     case WebKit::WebTextInputTypeTelephone:
      type = "Telephone";
      break;
     case WebKit::WebTextInputTypeURL:
      type = "URL";
      break;
#if defined(OS_ANDROID)
     case WebKit::WebTextInputTypeTextArea:
      type = "TextArea";
      break;
#endif
     default:
...
#if defined(OS_ANDROID)
template <>
struct ParamTraits<webkit_glue::MediaMetadataAndroid> {
  typedef webkit_glue::MediaMetadataAndroid param_type;
  static void Write(Message* m, const param_type& p);
  static bool Read(const Message* m, void** iter, param_type* r);
  static void Log(const param_type& p, std::string* l);
};
