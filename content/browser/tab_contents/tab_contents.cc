...
ViewMsg_Navigate_Type::Value GetNavigationType(
  ...
  switch (reload_type) {
    ...
    case NavigationControllerImpl::RELOAD_WITH_ORIGINAL_URL:
      return ViewMsg_Navigate_Type::RELOAD_WITH_ORIGINAL_URL;
...
void MakeNavigateParams(const NavigationEntryImpl& entry,
  ...
  if (reload_type == NavigationControllerImpl::RELOAD_WITH_ORIGINAL_URL) {
    // We may have been redirected when navigating to the current URL.
    // Use the URL the user originally intended to visit.
    params->url = entry.GetInitialURL();
  }
  ...
  params->user_agent_override = entry.GetUserAgentOverride();
...
double TabContents::GetLoadProgress() const {
...
bool TabContents::NavigateToEntry(
  ...
  // Generate the parameters for the navigation.
...
#if defined(OS_ANDROID)
void TabContents::SetAllowContentUrlAccess(bool allow) {
  allow_content_url_access_ = allow;
}

bool TabContents::GetAllowContentUrlAccess() const {
  return allow_content_url_access_;
}

void TabContents::SetOnlyAllowFileAccessToAndroidResources(
    bool only_android_res) {
  only_allow_file_access_to_android_resources_ = only_android_res;
}

bool TabContents::GetOnlyAllowFileAccessToAndroidResources() const {
  return only_allow_file_access_to_android_resources_;
}
#endif
...
#if defined(OS_ANDROID)
void TabContents::OnReceiveFindMatchRectsReply(
    int version,
    const std::vector<FindMatchRect>& rects,
    const FindMatchRect& active_rect) {
  delegate_->OnReceiveFindMatchRects(version, rects, active_rect);
}
#endif
...
void TabContents::RenderViewGone(RenderViewHost* rvh,
  ...
  GetView()->OnTabCrashed(GetCrashedStatus(), crashed_error_code_, handle);
...
WebPreferences TabContents::GetWebkitPrefs() {
  ...
  if ((GetURL().SchemeIs(chrome::kChromeDevToolsScheme) ||
    ...
    // Allow accelerated compositing for chrome: on Android as Android uses
...
#if defined(OS_ANDROID)
  // Frame flattening (expanding the size of overflow:scroll elements)
  // causes serious performance bugs on some pages, so hide it behind
  // a flag.
  static bool frame_flattening = CommandLine::ForCurrentProcess()->HasSwitch(
      switches::kFrameFlattening);
  if (frame_flattening)
    web_prefs.frame_flattening_enabled = true;

  web_prefs.allow_content_url_access = allow_content_url_access_;
  web_prefs.only_allow_file_access_to_android_resources =
...
void TabContents::RenderViewGoneFromRenderManager(
  ...
  RenderViewGone(render_view_host, crashed_status_, crashed_error_code_, 0);
}
...
bool TabContents::CreateRenderViewForRenderManager(
  ...
  // Determine what user agent override we're using.
  std::string user_agent_override;
  const NavigationEntry* entry = controller_.GetActiveEntry();
  if (entry)
    user_agent_override = entry->GetUserAgentOverride();
