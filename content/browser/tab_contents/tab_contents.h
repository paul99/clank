...
class CONTENT_EXPORT TabContents
 ...
 public:
  ...
  // 1 (page fully loaded).
  virtual double GetLoadProgress() const OVERRIDE;
...
#if defined(OS_ANDROID)
  virtual void SetAllowContentUrlAccess(bool allow) OVERRIDE;
  virtual bool GetAllowContentUrlAccess() const OVERRIDE;

  virtual void SetOnlyAllowFileAccessToAndroidResources(
      bool only_android_res) OVERRIDE;
  virtual bool GetOnlyAllowFileAccessToAndroidResources() const OVERRIDE;
#endif
 ...
 private:
  ...
  void OnFindReply(int request_id, int number_of_matches,
                   ...
                   bool final_update);
#if defined(OS_ANDROID)
  void OnReceiveFindMatchRectsReply(
    int version,
  ...
  // Restrict file:// URLs to file://android_{res,asset}/
