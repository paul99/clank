...
void InterstitialPage::Hide() {
  ...
  // TODO(jknotten): Upstream, in http://codereview.chromium.org/8430023/, this
  // notification is called just before 'delete this'.  We need to either move
  // upstream's call to here, or move our call to the same place as
  // downstream. Also, upstream the notification is unconditional.
  // Tracking this issue in http://b/5970753
  if (tab_->GetInterstitialPage()) {
    // Notify right before removing it, so it can still be accessed by the
    // observer.
...
WebContentsView* InterstitialPage::CreateWebContentsView() {
  ...
  const NavigationEntry* entry = tab()->GetController().GetActiveEntry();
  if (entry)
...
#if defined(OS_ANDROID)
void InterstitialPage::InterstitialPageRVHViewDelegate::OnReceiveFindMatchRects(
