...
const GURL& NavigationEntryImpl::GetInitialURL() const {
  if (original_request_url_.is_valid()) {
...
void NavigationEntryImpl::SetUserAgentOverride(const std::string& override) {
  user_agent_override_ = override;
}

const std::string& NavigationEntryImpl::GetUserAgentOverride() const {
