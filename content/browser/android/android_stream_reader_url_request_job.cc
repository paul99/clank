...
#include "jni/input_stream_jni.h"
...
bool AndroidStreamReaderURLRequestJob::VerifyRequestedRange(JNIEnv* env) {
  int64 size = static_cast<int64>(Java_InputStream_available(env,
                                                             stream_.obj()));
...
bool AndroidStreamReaderURLRequestJob::SkipToRequestedRange(JNIEnv* env) {
  ...
  if (byte_range_.first_byte_position() != 0) {
    int64 skipped, bytes_to_skip = byte_range_.first_byte_position();
    do {
      skipped = static_cast<int64>(Java_InputStream_skip(env, stream_.obj(),
                                                         bytes_to_skip));
      if (ClearException(env)) {
        NotifyDone(net::URLRequestStatus(net::URLRequestStatus::FAILED,
                                         net::ERR_FAILED));
...
bool AndroidStreamReaderURLRequestJob::ReadRawData(net::IOBuffer* dest,
  ...
  jbyteArray buffer = static_cast<jbyteArray>(buffer_.obj());
  ...
  *bytes_read = 0;
  ...
  while (dest_size > 0) {
    ...
    int byte_count = static_cast<int>(
...
#ifndef NDEBUG
    int buffer_length = static_cast<int>(env->GetArrayLength(buffer));
