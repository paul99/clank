...
class CONTENT_EXPORT RenderWidgetHost : public IPC::Channel::Listener,
 ...
 public:
  ...
  void ForwardVSyncEvent(base::TimeTicks frameBeginTime,
 ...
 private:
  ...
  void OnMsgRenderViewGone(int status,
                           int error_code,
  ...
  // Like Mac, we want to query the window size in the UI thread on Android.
//  void OnMsgGetWindowRect(gfx::NativeViewId window_id, gfx::Rect* results);
//  void OnMsgGetRootWindowRect(gfx::NativeViewId window_id, gfx::Rect* results);
  void OnMsgShowPressStateAck(const std::vector<gfx::Rect>& rects);
  // This is called when the java view needs to show the on-demand-zoom popup.
  // The DIB contains scaled up bitmap data so it can be drawn at full
  // resolution, instead of scaling the bitmap data to a lower resolution.
  void OnMsgMultipleTargetsTouched(const gfx::Rect& rect,
                                   const gfx::Size& size,
                                   const TransportDIB::Id& id);
  void OnMsgDidChangePageScaleFactorLimits(float minimum_scale,
                                           float maximum_scale);
  // |params| holds information on the currently selected input field.
  void OnMsgTouchEndAck(
      const ViewHostMsg_TextInputState_Params& params);

  void OnMsgSelectRangeAck();

  void OnMsgUpdateFrameInfo(const gfx::Point& scroll_offset,
...
#if defined(OS_ANDROID)
  // Throttling text selection IPC, allowing at most one unACKed IPC inflight.
  gfx::Point pend_select_start_;
  gfx::Point pend_select_end_;
  bool select_range_inflight_;
