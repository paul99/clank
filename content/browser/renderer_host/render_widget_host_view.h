...
#if defined(OS_ANDROID)
  virtual void ShowPressStateAck(const std::vector<gfx::Rect>& rects) = 0;
  virtual void ProcessTouchEndAck(
      const ViewHostMsg_TextInputState_Params& params) = 0;
  virtual void MultipleTargetsTouched(const gfx::Rect& rect,
                                      const gfx::Size& size,
                                      const TransportDIB* dib) = 0;
  virtual void SetGPUProcess(base::ProcessHandle handle) = 0;
  virtual void NativeWindowChanged(bool attached) = 0;
  virtual void CompositorResized(uint64 timestamp, const gfx::Size& size) = 0;
...
#if defined(OS_ANDROID)
  virtual void AcceleratedSurfaceNew(int route_id, int host_id) = 0;
#endif
