...
#if defined(OS_ANDROID)
        download_file_manager_(new DownloadFileManagerAndroid(this))),
#else
...
const char ResourceDispatcherHost::kXAutoLogin[] = "X-Auto-Login";

void ResourceDispatcherHost::CheckForAutoLogin(net::URLRequest* request) {
  net::HttpResponseHeaders* headers = request->response_headers();
  std::string value;
  if (headers &&
      headers->EnumerateHeader(NULL, kXAutoLogin, &value)) {
    VLOG(0) << "x-auto-login: " << value;

    int render_process_id, tab_contents_id;
    if (!RenderViewForRequest(request, &render_process_id, &tab_contents_id))
      return;

    content::GetContentClient()->browser()->CreateAutoLogin(render_process_id,
       tab_contents_id, value);
  }
}
