...
struct ViewHostMsg_SelectionChanged_Params;
...
class CONTENT_EXPORT RenderViewHost : public RenderWidgetHost {
 public:
...
#if defined(OS_ANDROID)
  // Set the GPU process handle associated with a given view.
  static void SetGPUProcess(int surface_id,
  ...
  void ActivateNearestFindResult(int request_id, float x, float y);
 ...
 protected:
  ...
  void ApplyContentAccessSetting();
#endif
...
#if defined(OS_ANDROID)
  void OnMsgDidSetNeedTouchEvents(bool need_touch_events);
  void OnMsgDidChangeBodyBackgroundColor(SkColor color);
#endif
#if defined(OS_MACOSX)
  // Added in a distinct location so it gets found in a diff that I'm
  // bouncing between trees.
  void OnMsgDidContentSizeChange(const gfx::Size& size);
#endif
