...
#if !defined(TOOLKIT_USES_GTK) && !defined(OS_ANDROID)
    // In-process plugins require this to be a UI message loop.
    options.message_loop_type = MessageLoop::TYPE_UI;
#else
    // We can't have multiple UI loops on GTK or Android, so we don't support
    // in-process plugins.
...
#if defined(OS_ANDROID)
    ChildProcessHostImpl::AppendCommonChildCommandLine(cmd_line);
    ChildProcessHostImpl::FillChildEnvironment(&env);
#endif
...
bool RenderProcessHostImpl::OnMessageReceived(const IPC::Message& msg) {
  ...
  if (msg.routing_id() == MSG_ROUTING_CONTROL) {
    ...
    IPC_BEGIN_MESSAGE_MAP_EX(RenderProcessHostImpl, msg, msg_is_ok)
      ...
      // TODO(clank): at this time (9/29/10), the browser receives
      // unhandled messages from the render, such as
      // ViewHostMsg_Geolocation_RegisterDispatcher.  Before we can fix
      // make NOTREACHED(), we need to handle (or prevent the sending of)
      // these messages.
