...
bool RenderWidgetHost::OnMessageReceived(const IPC::Message &msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP_EX(RenderWidgetHost, msg, msg_is_ok)
...
#if defined(OS_ANDROID)
    IPC_MESSAGE_HANDLER(ViewHostMsg_HandleTouchEnd_ACK, OnMsgTouchEndAck)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_ShowPressState_ACK, OnMsgShowPressStateAck)
    IPC_MESSAGE_HANDLER(ViewHostMsg_MultipleTargetsTouched,
                        OnMsgMultipleTargetsTouched)
    IPC_MESSAGE_HANDLER(ViewHostMsg_DidChangePageScaleFactorLimits,
                        OnMsgDidChangePageScaleFactorLimits)
...
void RenderWidgetHost::ForwardVSyncEvent(
      base::TimeTicks frameBeginTime, base::TimeDelta currentFrameInterval) {
  TRACE_EVENT0("renderer_host", "RenderWidgetHost::ForwardVSyncEvent");
  Send(new ViewMsg_DidVSync(
      routing_id_,
      frameBeginTime.ToInternalValue(),
      currentFrameInterval.ToInternalValue()));
}
...
#if defined(OS_ANDROID)
void RenderWidgetHost::OnMsgTouchEndAck(
    const ViewHostMsg_TextInputState_Params& params) {
  if (view_)
    view_->ProcessTouchEndAck(params);
}

void RenderWidgetHost::OnMsgSelectRangeAck() {
  select_range_inflight_ = false;
...
void RenderWidgetHost::OnMsgTextInputStateChanged(
    const ViewHostMsg_TextInputState_Params& params) {
  if (view_)
    view_->ImeUpdateTextInputState(params);
}
#else
void RenderWidgetHost::OnMsgTextInputStateChanged(
...
#if defined(OS_ANDROID)
void RenderWidgetHost::OnMsgShowPressStateAck(
    const std::vector<gfx::Rect>& rects) {
  if (view_)
    view_->ShowPressStateAck(rects);
}

void RenderWidgetHost::OnMsgMultipleTargetsTouched(
    const gfx::Rect& rect,
    const gfx::Size& size,
    const TransportDIB::Id& id) {
  if (view_) {
    TransportDIB* dib = process_->GetTransportDIB(id);
    view_->MultipleTargetsTouched(rect, size, dib);
    Send(new ViewMsg_ReleaseOnDemandDIB(routing_id(), dib->handle()));
  }
}

void RenderWidgetHost::OnMsgDidChangePageScaleFactorLimits(
                                                   float minimum_scale,
                                                   float maximum_scale) {
  if (view_)
    view_->SetCachedPageScaleFactorLimits(minimum_scale, maximum_scale);
}

void RenderWidgetHost::OnMsgUpdateFrameInfo(const gfx::Point& scroll_offset,
