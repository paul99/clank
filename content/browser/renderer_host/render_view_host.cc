...
#if defined(OS_ANDROID)
// static
void RenderViewHost::SetGPUProcess(int surface_id,
                                   base::ProcessHandle handle) {
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
  int render_process_id = 0;
  int render_view_id = 0;
  if (!GpuSurfaceTracker::Get()->GetRenderWidgetIDForSurface(
        surface_id, &render_process_id, &render_view_id))
    return;
  RenderViewHost* host =
      RenderViewHost::FromID(render_process_id, render_view_id);
  if (host && host->view())
    host->view()->SetGPUProcess(handle);
}
#endif
...
#if defined(OS_ANDROID)
void RenderViewHost::ApplyContentAccessSetting() {
  const WebPreferences webkit_prefs = delegate()->GetWebkitPrefs();

  if (webkit_prefs.allow_content_url_access) {
    ChildProcessSecurityPolicy::GetInstance()->GrantScheme(
        process()->GetID(), chrome::kContentScheme);
  } else {
    ChildProcessSecurityPolicy::GetInstance()->RevokeScheme(
        process()->GetID(), chrome::kContentScheme);
  }
}
#endif
...
#if defined(OS_ANDROID)
void RenderViewHost::ActivateNearestFindResult(int request_id,
                                               float x, float y) {
  Send(new ViewMsg_ActivateNearestFindResult(routing_id(), request_id, x, y));
}

void RenderViewHost::RequestFindMatchRects(int have_version) {
  Send(new ViewMsg_GetFindMatchRects(routing_id(), have_version));
}
#endif
...
#if defined(OS_ANDROID)
void RenderViewHost::OrientationChange(int orientation) {
  Send(new ViewMsg_OrientationChange(routing_id(), orientation));
}
#endif
...
#if defined(OS_ANDROID)
    IPC_MESSAGE_HANDLER(ViewHostMsg_DidSetNeedTouchEvents,
                        OnMsgDidSetNeedTouchEvents)
...
void RenderViewHost::OnMsgNavigate(const IPC::Message& msg) {
  ...
  FilterURL(policy, renderer_id, true, &validated_params.original_request_url);
...
#if defined(OS_ANDROID)
void RenderViewHost::OnMsgShouldIgnoreNavigation(
    const GURL& url,
    const content::Referrer& referrer,
    WindowOpenDisposition disposition,
...
#if defined(OS_ANDROID)
void RenderViewHost::OnMsgDidSetNeedTouchEvents(bool need_touch_events) {
  RenderViewHostDelegate::View* view = delegate_->GetViewDelegate();
  if (view)
    view->DidSetNeedTouchEvents(need_touch_events);
}

void RenderViewHost::OnMsgDidChangeBodyBackgroundColor(SkColor color) {
  if (view())
    view()->SetCachedBackgroundColor(color);
}

void RenderViewHost::OnStartContentIntent(const GURL& content_url) {
  if (view())
    view()->StartContentIntent(content_url);
}
...
void RenderViewHost::OnMsgSelectionBoundsChanged(
    ...
    WebKit::WebTextDirection start_dir,
    const gfx::Rect& end_rect,
    WebKit::WebTextDirection end_dir) {
  if (view())
    view()->SelectionBoundsChanged(start_rect, WebTextDirectionToChromeTextDirection(start_dir),
                                   end_rect, WebTextDirectionToChromeTextDirection(end_dir));
}
#else
void RenderViewHost::OnMsgSelectionBoundsChanged(
    const gfx::Rect& start_rect,
...
void RenderViewHost::OnMsgStartDragging(
  ...
  if (!drag_url.SchemeIs(chrome::kJavaScriptScheme))
    FilterURL(policy, process()->GetID(), true, &drag_url);
...
#if defined(OS_ANDROID)
void RenderViewHost::ConfirmTouchEvent(bool handled) {
  RenderViewHostDelegate::View* view = delegate_->GetViewDelegate();
  if (view)
    view->ConfirmTouchEvent(handled);
}
#endif
...
#if defined(OS_ANDROID)
void RenderViewHost::DidSelectPopupMenuItems(
    const std::vector<int>& selected_indices) {
  Send(new ViewMsg_SelectPopupMenuItems(routing_id(), false,
                                        selected_indices));
}

void RenderViewHost::DidCancelPopupMenu() {
  Send(new ViewMsg_SelectPopupMenuItems(routing_id(), true,
                                        std::vector<int>()));
}
#endif
