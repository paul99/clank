...
#if defined(OS_ANDROID)
  // Too many websites forward to flash files directly, expecting flash to be
  // supported. This results in downloads that are both unintented from the
  // website and a bad experience for the user. This disables downloads that
  // occur from navigating to a flash file from within an iframe. "Save link
  // as" is still allowed everywhere. Navigating to a flash file in the main
  // frame will still trigger a download.
  if (type == "application/x-shockwave-flash" && !info->is_main_frame())
      return false;
#endif
