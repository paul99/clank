...
void NotificationServiceImpl::Notify(
  ...
  if (HasKey(observers_[static_cast<int>(content::NOTIFICATION_ALL)], AllSources()) &&
      source != AllSources()) {
    FOR_EACH_OBSERVER(content::NotificationObserver,
       *observers_[static_cast<int>(content::NOTIFICATION_ALL)][AllSources().map_key()],
       Observe(type, source, details));
  }

  // Notify observers of all types and the given source
  if (HasKey(observers_[static_cast<int>(content::NOTIFICATION_ALL)], source)) {
    FOR_EACH_OBSERVER(content::NotificationObserver,
        *observers_[static_cast<int>(content::NOTIFICATION_ALL)][source.map_key()],
        Observe(type, source, details));
  }
