...
#if defined(OS_MACOSX) || defined(UI_COMPOSITOR_IMAGE_TRANSPORT) || defined(OS_ANDROID)
...
#if defined(OS_ANDROID)
    IPC_MESSAGE_HANDLER(GpuHostMsg_NativeWindowChanged, OnNativeWindowChanged)
...
void GpuProcessHostUIShim::OnAcceleratedSurfaceNew(
  ...
  view->AcceleratedSurfaceNew(params.route_id, host_id_);
...
#if defined(OS_ANDROID)
void GpuProcessHostUIShim::OnNativeWindowChanged(int32 surface_id,
                                                 bool attached) {
  RenderWidgetHostView* view = GetRenderWidgetHostViewFromSurfaceID(surface_id);
  if (!view)
    return;
  view->NativeWindowChanged(attached);
}

void GpuProcessHostUIShim::OnNotifyCompositorResize(
    int32 surface_id, uint64 timestamp, gfx::Size size) {
  RenderWidgetHostView* view = GetRenderWidgetHostViewFromSurfaceID(surface_id);
  if (view) {
    view->CompositorResized(timestamp, size);
  }
}
#endif
