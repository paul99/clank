...
void AndroidLocationApiAdapter::CreateJavaObject(JNIEnv* env) {
  jmethodID init = GetMethodID(env, g_LocationProvider_clazz, "<init>",
                               "(Landroid/content/Context;)V");
  // Create the Java AndroidLocationProvider object.
  ScopedJavaLocalRef<jobject> scoped_provider(env,
      env->NewObject(g_LocationProvider_clazz, init,
                     base::android::GetApplicationContext()));
  java_location_provider_android_object_.Reset(scoped_provider);
  CHECK(!java_location_provider_android_object_.is_null());
}
