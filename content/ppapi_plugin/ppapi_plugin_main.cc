...
int PpapiPluginMain(const content::MainFunctionParams& parameters) {
...
#if defined(OS_ANDROID)
    // TODO(huanr): switch to use WaitForDebugger once that is fixed
    // on Android.
    while(g_ppapi_wait_for_gdb) {
  ...
  // megamerge note: default is TYPE_DEFAULT which is what Clank wants.
