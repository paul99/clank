...
void WebUIBindings::Send(const CppArgumentList& args, CppVariant* result) {
  ...
  if (args.size() < 1 || args.size() > 2) {
    LOG(WARNING) << "Invalid send call, ignored. wrong number of parameters:" <<
        args.size();
    return;
  }
  ...
  if (!args[0].isString()) {
    LOG(WARNING) <<
        "Invalid send call, ignored. Parameter 0 is not a string.";
    return;
  }
  ...
  if (args.size() == 2) {
    if (!args[1].isObject()) {
      LOG(WARNING) << "Invalid send call '" << message << "', ignored. "
          "Parameter 1 is not an object.";
      return;
    }
