...
#if defined(OS_ANDROID)
      num_composition_changes_in_flight_(0),
...
void RenderWidget::OnHandleInputEvent(const IPC::Message& message) {
  ...
  if (input_event->type != WebInputEvent::Char || !suppress_next_char_events_) {
    ...
    if (!processed && webwidget_) {
      bool keyb_event = WebInputEvent::isKeyboardEventType(input_event->type);
      if (keyb_event)
...
#if defined(OS_ANDROID)
  // Allow the keyboard to be shown when the focus changes as a consequence
...
void RenderWidget::UpdateTextInputState() {
  ...
  gfx::Rect new_caret_bounds, end_rect;

  GetSelectionBounds(&new_caret_bounds, &end_rect);
  if (webwidget_) {
...
void RenderWidget::UpdateSelectionBounds() {
  ...
  Send(new ViewHostMsg_SelectionBoundsChanged(
      routing_id_, selection_start_rect_, start_dir, selection_end_rect_, end_dir));
#else
  Send(new ViewHostMsg_SelectionBoundsChanged(
...
COMPILE_ASSERT(int(WebKit::WebTextInputTypeDate) == \
               int(ui::TEXT_INPUT_TYPE_DATE), mismatching_enums);
COMPILE_ASSERT(int(WebKit::WebTextInputTypeDateTime) == \
               int(ui::TEXT_INPUT_TYPE_DATETIME), mismatching_enums);
COMPILE_ASSERT(int(WebKit::WebTextInputTypeDateTimeLocal) == \
               int(ui::TEXT_INPUT_TYPE_DATETIMELOCAL), mismatching_enums);
COMPILE_ASSERT(int(WebKit::WebTextInputTypeMonth) == \
               int(ui::TEXT_INPUT_TYPE_MONTH), mismatching_enums);
COMPILE_ASSERT(int(WebKit::WebTextInputTypeTime) == \
               int(ui::TEXT_INPUT_TYPE_TIME), mismatching_enums);
COMPILE_ASSERT(int(WebKit::WebTextInputTypeWeek) == \
               int(ui::TEXT_INPUT_TYPE_WEEK), mismatching_enums);
...
ui::TextInputType RenderWidget::WebKitToUiTextInputType
  ...
  DCHECK(type <= (int) ui::TEXT_INPUT_TYPE_MAX) <<
