...
CompositorThread::CompositorThread(IPC::Channel::Listener* main_listener)
  ...
  filter_ =
      new InputEventFilter(main_listener,
                           ...
                           base::Bind(&CompositorThread::HandleVSyncEvent,
                                      base::Unretained(this)));
  setpriority(PRIO_PROCESS, thread_.thread_id(), -2);
}
...
void CompositorThread::HandleVSyncEvent(
    int routing_id,
  ...
  double usecPerSecond =
      static_cast<double>(base::Time::kMicrosecondsPerSecond);
  it->second->compositor()->didVSync(frameBeginTimeUSec / usecPerSecond,
                                     currentFrameIntervalUSec / usecPerSecond);
}
