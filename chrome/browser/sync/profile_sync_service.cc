...
#include "chrome/browser/sync/util/experiments.h"
...
void ProfileSyncService::ClearSyncSetupCompleted() {
  sync_prefs_.ClearSyncSetupCompleted();
}
...
void ProfileSyncService::ShowSyncSetup(const std::string& sub_page) {
  ...
  if (!browser) {
#if defined(OS_ANDROID)
    NOTIMPLEMENTED() << "Clank doesn't have Browser::Create!";
#else
    ...
    case chrome::NOTIFICATION_TOKEN_AVAILABLE: {
      const TokenService::TokenAvailableDetails& token_details =
          *(content::Details<const TokenService::TokenAvailableDetails>(
              details).ptr());
      // Currently app notifications require an oauth2 token for syncing so
      // when checking credentials we check for both a sync token and an
      // oauth2 token. However, Android doesn't have apps/app-notifications
      // and furthermore, Android jams the sync token in the token service
      // which Desktop chrome doesn't. Once signin code is refactored
      // completely out of sync, and app notifications don't rely on sync
      // being signed in, this should be merged back.
#if defined(OS_ANDROID)
      bool require_oath2_token = false;
#else
      bool require_oath2_token = true;
#endif
      if (IsTokenServiceRelevant(token_details.service()) &&
          AreCredentialsAvailable(require_oath2_token)) {
