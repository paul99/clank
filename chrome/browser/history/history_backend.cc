...
SegmentID HistoryBackend::UpdateSegments(
  ...
  // Note that navigating to an existing entry (with back/forward) resuses the
...
void HistoryBackend::InitImpl(const std::string& languages, Profile* profile) {
...
void HistoryBackend::AddBookmark(scoped_refptr<InsertRequest> request,
  ...
  if (android_provider_backend_.get())
    id = android_provider_backend_->Insert(row);
...
void HistoryBackend::QueryBookmark(
  ...
  if (android_provider_backend_.get())
    statement = android_provider_backend_->Query(projections, selection,
        selection_args, sort_order);

  request->ForwardResult(request->handle(), statement, statement);

  // The android_urls table could be synced before querying.
  ScheduleCommit();
}

void HistoryBackend::UpdateBookmark(
  ...
  int count = 0;
  if (android_provider_backend_.get())
    count = android_provider_backend_->Update(row, selection, selection_args);

  request->ForwardResult(request->handle(), count, count);

  ScheduleCommit();
}

void HistoryBackend::DeleteBookmark(
  ...
  int count = 0;
  if (android_provider_backend_.get())
    count = android_provider_backend_->Delete(selection, selection_args);
...
void HistoryBackend::DeleteHistory(
  ...
  int count = 0;
  if (android_provider_backend_.get())
    count = android_provider_backend_->DeleteHistory(selection, selection_args);
...
void HistoryBackend::MoveStatement(
    ...
    const AndroidProviderService::StatementRequest& statement) {
  int start_pos = statement.from;
  int end_pos = statement.to;
  if (start_pos > end_pos) {
    statement.statement->statement()->ResetWithoutClearingBoundVariables();
    start_pos = -1;
  }
  int i = start_pos;
  for (; i< end_pos; ++i)
    if (!statement.statement->statement()->Step())
      break;

  request->ForwardResult(request->handle(), statement.statement, i);
}
...
void HistoryBackend::BindBlob(scoped_refptr<BindBlobRequest> request,
                              LazyBindingColumn* column) {
  std::vector<unsigned char> data;
  column->ColumnBlobAsVector(&data);
  return request->ForwardResult(request->handle(), data);
}

void HistoryBackend::BindString(scoped_refptr<BindStringRequest> request,
                                LazyBindingColumn* column) {
  return request->ForwardResult(request->handle(),
                                column->ColumnString16());
}

void HistoryBackend::AddSearchTerm(scoped_refptr<InsertRequest> request,
                                   const SearchRow& row) {
  if (request->canceled())
    return;
  ...
  URLID id = 0;
  if (android_provider_backend_.get()) {
    // Android CTS allowed the same search term being inserted multiple times.
    id = android_provider_backend_->FindSearchTerm(row.search_term());
    if (id) {
      std::vector<history::VisitInfo> time;
      ...
      time.push_back(history::VisitInfo(row.search_time(),
                                        content::PAGE_TRANSITION_TYPED));
      AddVisits(row.url(), time, SOURCE_BROWSED);
    } else {
      std::vector<history::VisitInfo> time;
      time.push_back(history::VisitInfo(row.search_time(),
                                        content::PAGE_TRANSITION_TYPED));
      if (AddVisits(row.url(), time, SOURCE_BROWSED)) {
        SetKeywordSearchTermsForURL(row.url(), row.template_url_id(),
                                    row.search_term());
        URLRow url_row;
        if (GetURL(row.url(), &url_row))
          id = url_row.id();
        else
          LOG(ERROR) << "Can not find URL";
      } else {
        LOG(ERROR) << "Can not add url " << row.url();
      }
    }
  } else {
    LOG(ERROR) << "Search term already exists or service is not available";
  }
  request->ForwardResult(request->handle(), id != 0, id);
...
void HistoryBackend::UpdateSearchTerm(
    scoped_refptr<UpdateRequest> request,
    const SearchRow& row,
    const std::string& selection,
    const std::vector<string16> selection_args) {
  ...
  int count = 0;
  if (android_provider_backend_.get()) {
    count =  android_provider_backend_->UpdateSearchTerm(row, selection,
                                                         selection_args);
  }
  request->ForwardResult(request->handle(), count != 0, count);

  ScheduleCommit();
}

void HistoryBackend::DeleteSearchTerm(
    scoped_refptr<DeleteRequest> request,
    const std::string& selection,
    const std::vector<string16> selection_args) {
  if (request->canceled())
  ...
  int count = 0;
  if (android_provider_backend_.get()) {
    count =  android_provider_backend_->DeleteSearchTerm(selection,
                                                         selection_args);
  }
  request->ForwardResult(request->handle(), count != 0, count);
...
void HistoryBackend::QuerySearchTerm(
    scoped_refptr<QueryRequest> request,
  ...
  AndroidStatement* statement = NULL;
  if (android_provider_backend_.get())
    statement = android_provider_backend_->QuerySearchTerm(projections,
...
#if defined(OS_ANDROID)
  // TODO (michaelbai): Find out why there is no notification sent out in this
  // method.
  android_provider_backend_->ClearAndroidURLsTable();
#endif
