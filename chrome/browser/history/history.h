...
class HistoryService : public CancelableRequestProvider,
 ...
 private:
  ...
  // Add the given BookmarkRow, and return the result through request's
  // callback.
  void AddBookmark(AndroidProviderService::InsertRequest* request,
  ...
  void UpdateBookmark(AndroidProviderService::UpdateRequest* request,
                      const history::BookmarkRow& row,
                      const std::string& selection,
                      const std::vector<string16>& selection_args);

  void DeleteBookmark(AndroidProviderService::DeleteRequest* request,
                      const std::string& selection,
                      const std::vector<string16>& selection_args);
  ...
  // Move the give statement |handle| from |from| to |to|.
  ...
  // Close the statement in db thread.
  void CloseStatement(history::AndroidStatement* statement);

  // Bind the given column as blob in db thread.
  void BindBlob(AndroidProviderService::BindBlobRequest* request,
                history::LazyBindingColumn* column);

  // Bind the given column as string in db thread.
  void BindString(AndroidProviderService::BindStringRequest* request,
                  history::LazyBindingColumn* column);

  // Add the given SearchTermRow, and return the result through request's
  // callback.
  void AddSearchTerm(AndroidProviderService::InsertRequest* request,
                     const history::SearchRow& row);

  void UpdateSearchTerm(AndroidProviderService::UpdateRequest* request,
                        const history::SearchRow& row,
                        const std::string& selection,
                        const std::vector<string16>& selection_arg);

  void DeleteSearchTerm(AndroidProviderService::DeleteRequest* request,
                        const std::string& selection,
                        const std::vector<string16>& selection_arg);
