...
void HistoryService::AddBookmark(AndroidProviderService::InsertRequest* request,
                                 const history::BookmarkRow& row) {
  Schedule(PRIORITY_NORMAL, &HistoryBackend::AddBookmark, NULL, request, row);
}

void HistoryService::QueryBookmark(
  ...
  Schedule(PRIORITY_NORMAL, &HistoryBackend::QueryBookmark, NULL, request,
           projections, selection, selection_args, sort_order);
}

void HistoryService::UpdateBookmark(
    AndroidProviderService::UpdateRequest* request,
    const history::BookmarkRow& row,
    const std::string& selection,
    const std::vector<string16>& selection_args) {
  Schedule(PRIORITY_NORMAL, &HistoryBackend::UpdateBookmark, NULL, request,
           row, selection, selection_args);
}

void HistoryService::DeleteBookmark(
    AndroidProviderService::DeleteRequest* request,
    const std::string& selection,
    const std::vector<string16>& selection_args) {
  Schedule(PRIORITY_NORMAL, &HistoryBackend::DeleteBookmark, NULL, request,
...
void HistoryService::BindBlob(
    AndroidProviderService::BindBlobRequest* request,
    history::LazyBindingColumn* column) {
  Schedule(PRIORITY_NORMAL, &HistoryBackend::BindBlob, NULL, request, column);
}

void HistoryService::BindString(
    AndroidProviderService::BindStringRequest* request,
    history::LazyBindingColumn* column) {
  Schedule(PRIORITY_NORMAL, &HistoryBackend::BindString, NULL, request, column);
}

void HistoryService::AddSearchTerm(
    AndroidProviderService::InsertRequest* request,
    const history::SearchRow& row) {
  Schedule(PRIORITY_NORMAL, &HistoryBackend::AddSearchTerm, NULL, request, row);
}

void HistoryService::UpdateSearchTerm(
    AndroidProviderService::UpdateRequest* request,
    const history::SearchRow& row,
    const std::string& selection,
    const std::vector<string16>& selection_arg) {
  Schedule(PRIORITY_NORMAL, &HistoryBackend::UpdateSearchTerm, NULL, request,
           row, selection, selection_arg);
}

void HistoryService::DeleteSearchTerm(
    AndroidProviderService::DeleteRequest* request,
    const std::string& selection,
    const std::vector<string16>& selection_arg) {
  Schedule(PRIORITY_NORMAL, &HistoryBackend::DeleteSearchTerm, NULL, request,
           selection, selection_arg);
}

void HistoryService::QuerySearchTerm(
    AndroidProviderService::QueryRequest* request,
    const std::vector<history::SearchRow::SearchColumnID>& projections,
    const std::string& selection,
    const std::vector<string16>& selection_args,
    const std::string& sort_order) {
  Schedule(PRIORITY_NORMAL, &HistoryBackend::QuerySearchTerm, NULL, request,
           projections, selection, selection_args, sort_order);
}
