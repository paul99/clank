...
class HistoryBackend : public base::RefCountedThreadSafe<HistoryBackend>,
 ...
 public:
  ...
  // |profile| The profile to associate history with.
  void Init(const std::string& languages, bool force_fail, Profile* profile);
  ...
  // callback.
  void AddBookmark(scoped_refptr<InsertRequest> request,
  ...
  void UpdateBookmark(scoped_refptr<UpdateRequest> request,
                      const BookmarkRow& row,
                      const std::string& selection,
                      const std::vector<string16>& selection_args);

  void DeleteBookmark(scoped_refptr<DeleteRequest> request,
                      const std::string& selection,
                      const std::vector<string16>& selection_args);
  ...
  void BindBlob(scoped_refptr<BindBlobRequest> request,
                LazyBindingColumn* column);

  void BindString(scoped_refptr<BindStringRequest> request,
                  LazyBindingColumn* column);

  void AddSearchTerm(scoped_refptr<InsertRequest> request,
                     const SearchRow& row);

  void UpdateSearchTerm(scoped_refptr<UpdateRequest> request,
                        const SearchRow& row,
                        const std::string& selection,
                        const std::vector<string16> selection_args);

  void DeleteSearchTerm(scoped_refptr<DeleteRequest> request,
                        const std::string& selection,
                        const std::vector<string16> selection_args);

  void QuerySearchTerm(scoped_refptr<QueryRequest> request,
 ...
 private:
  ...
  void InitImpl(const std::string& languages, Profile* profile);
...
#if defined(OS_ANDROID)
  scoped_refptr<AndroidProviderBackend> android_provider_backend_;
#endif
