...
#if defined(OS_ANDROID)
void PrerenderRenderWidgetHostView::ShowPressStateAck(
    const std::vector<gfx::Rect>& rects, uint32 tap_highlight_color) {
  NOTREACHED();
}

void PrerenderRenderWidgetHostView::ProcessTouchEndAck(
    const ViewHostMsg_TextInputState_Params& params) {
  NOTREACHED();
}

void PrerenderRenderWidgetHostView::MultipleTargetsTouched(
    const gfx::Rect& rect,
    const gfx::Size& size,
...
void PrerenderRenderWidgetHostView::SelectionChanged(
...
void PrerenderRenderWidgetHostView::OnAcceleratedCompositingStateChange() {
}
