...
SessionCommand* BaseSessionService::CreateUpdateTabNavigationCommand(
  ...
  WriteStringToPickle(pickle, &bytes_written, max_state_size,
      entry.GetInitialURL().is_valid() ?
          entry.GetInitialURL().spec() : std::string());
...
bool BaseSessionService::RestoreUpdateTabNavigationCommand(
  ...
  if (has_type_mask) {
    ...
    if (pickle->ReadInt(&iterator, &policy_int)) {
      ...
      // The user agent override info comes after these.
      std::string url_spec;
      bool user_agent_overridden;
      if (!pickle->ReadString(&iterator, &url_spec) ||
          !pickle->ReadBool(&iterator, &user_agent_overridden)) {
        url_spec = "";
        user_agent_overridden = false;
      }
      navigation->set_initial_url(GURL(url_spec));
      navigation->set_user_agent_overridden(user_agent_overridden);
    } else {
