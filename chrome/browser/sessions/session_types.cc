...
TabNavigation::TabNavigation(const TabNavigation& tab)
    : virtual_url_(tab.virtual_url_),
      ...
      index_(tab.index_),
      initial_url_(tab.initial_url_),
      user_agent_overridden_(tab.user_agent_overridden_) {
}
...
NavigationEntry* TabNavigation::ToNavigationEntry(
  ...
  entry->SetOriginalRequestURL(initial_url_);
  entry->SetUserAgentOverride(user_agent_overridden_ ?
      webkit_glue::GetDesktopUserAgent() : std::string());
...
void TabNavigation::SetFromNavigationEntry(const NavigationEntry& entry) {
  ...
  initial_url_ = entry.GetInitialURL();
  user_agent_overridden_ = !entry.GetUserAgentOverride().empty();
}
