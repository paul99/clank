...
void CrashDumpTask(CrashHandlerHostLinux* handler, BreakpadInfo* info) {
  if (handler->IsShuttingDown())
    return;
  LOG(INFO) << "CrashDumpTask - info->filename: " << info->filename;
...
void CrashHandlerHostLinux::OnFileCanReadWithoutBlocking(int fd) {
  ...
  LOG(INFO) << "A process has crashed.";
...
void CrashHandlerHostLinux::WriteDumpFile(BreakpadInfo* info,
  ...
  // We need to also include the crashing_pid in the breakpad info. It should
  // first be converted to a string because at this point we know we are safely
  // processing non-browser crashes in the browser thread.
  std::stringstream ss;
  ss << crashing_pid;
  std::string tmp(ss.str());
  // Deleted in CrashTask with other BreakpadInfo fields.
  char* buf = new char[tmp.size()];
  // Shouldn't assign a c_str need to copy it.
  info->pid_str = strcpy(buf, tmp.c_str());
  info->pid_str_length = tmp.size();;
