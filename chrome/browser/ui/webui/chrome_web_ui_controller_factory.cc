...
WebUIFactoryFunction GetWebUIFactoryFunction(content::WebUI* web_ui,
  ...
  if (url.host() == chrome::kChromeUINewTabHost ||
      url.SchemeIs(chrome::kChromeInternalScheme))
#if defined(OS_ANDROID)
    return &NewWebUI<MobileNewTabUI>;
#else
...
#endif
#if !defined(ANDROID_BINSIZE_HACK)
  // TODO(qinmin): BugReportUI was renamed to FeedbackUI during merge.
  // Should we still disable it in clank?
...
#if !defined(ANDROID_BINSIZE_HACK)
  // Clank doesn't use the Chrome settings pages or the plugin config page.
