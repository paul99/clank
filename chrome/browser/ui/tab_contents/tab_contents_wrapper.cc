...
#if !defined(ANDROID_BINSIZE_HACK)
  // Web intents require BrowserNavigator that we haven't implemented
...
// For testing, Clank-only, please upstream.

#if defined (OS_ANDROID)
void TabContentsWrapper::DisableExternalAutofillDelegate() {
  if (autofill_external_delegate_.get()) {
    autofill_manager_->SetExternalDelegate(NULL);
    autocomplete_history_manager_->SetExternalDelegate(NULL);
    autofill_external_delegate_.reset();
  }
}
#endif
