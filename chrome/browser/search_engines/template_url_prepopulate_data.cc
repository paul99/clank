...
#define GOOGLE_SOURCE_ID L"chrome-mobile"
#else
#define GOOGLE_SOURCE_ID L"chrome"
#endif
...
const PrepopulatedEngine google = {
  ...
  L"{google:baseURL}search?{google:RLZ}{google:acceptedSuggestion}"
      ...
      L"{google:instantFieldTrialGroupParameter}{google:searchClient}"
      L"sourceid=" GOOGLE_SOURCE_ID L"&ie={inputEncoding}&q={searchTerms}",
  "UTF-8",
  L"{google:baseSuggestURL}search?{google:searchFieldtrialParameter}"
      L"{google:instantFieldTrialGroupParameter}"
      L"client=" GOOGLE_SOURCE_ID L"&hl={language}&q={searchTerms}",
...
int GetCurrentCountryID() {
  if (browser_defaults::kCountryCodeAtInstall != "")
    return CountryCharsToCountryIDWithUpdate(
        browser_defaults::kCountryCodeAtInstall[0],
        browser_defaults::kCountryCodeAtInstall[1]);
    return kCountryIDUnknown;
}
