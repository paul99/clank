...
void AutocompleteHistoryManager::SendSuggestions(
  ...
  // TODO(qinmin): this should belong to the else statement if we are not using
  // external delegate. However, the upstream haven't implemented suggestion
  // selection. So we still need to rely on webkit to store all the autofill
  // suggestions. Otherwise, when we send an listIndex to the webkit, we will
  // not have an AutofillPopupMenuClient which is responsible for retrieving
  // stored form values.
