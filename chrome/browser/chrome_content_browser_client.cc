...
content::BrowserMainParts* ChromeContentBrowserClient::CreateBrowserMainParts(
  ...
  ChromeBrowserMainParts* main_parts = NULL;
  ...
  // TODO(chrome-mobile): Android initialization should use the
...
#if defined(OS_ANDROID)
    std::string distro("Android");
#else
    std::string distro(base::GetLinuxDistro());
#endif
    command_line->AppendSwitchASCII(switches::kEnableCrashReporter,
        child_process_logging::GetClientId() + "," + distro);
  }
#elif defined(OS_MACOSX)
  if (IsCrashReporterEnabled()) {
    command_line->AppendSwitchASCII(switches::kEnableCrashReporter,
...
WebPreferences ChromeContentBrowserClient::GetWebkitPrefs(RenderViewHost* rvh) {
  ...
  {  // Command line switches are used for preferences with no user interface.
    ...
    web_prefs.threaded_animation_enabled =
...
#if defined(OS_ANDROID)
    if (command_line.HasSwitch(switches::kFontBoostingVersion)) {
      int value;
      base::StringToInt(command_line.GetSwitchValueASCII(
          switches::kFontBoostingVersion), &value);
      web_prefs.font_boosting_version = value;
    }
...
#if !defined(OS_ANDROID)
  // Android doesn't have extensions
  ExtensionService* service = profile->GetExtensionService();
  if (service) {
...
#if defined(OS_ANDROID)
void ChromeContentBrowserClient::CreateAutoLogin(
    int render_process_id,
    int web_contents_id,
