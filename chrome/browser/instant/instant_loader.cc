...
void InstantLoader::TabContentsDelegateImpl::Observe(
  ...
  switch (type) {
    ...
    case content::NOTIFICATION_INTERSTITIAL_ATTACHED:
      ...
      if (loader_->delegate_) {
        loader_->delegate_->InterstitialCreated();
      }
#endif
      PreviewPainted();
      break;
...
void InstantLoader::TabContentsDelegateImpl::DidNavigateMainFramePostCommit(
    content::WebContents* tab) {
#if defined(OS_ANDROID)
  loader_->main_frame_committed_ = true;
#endif
}
...
bool InstantLoader::Update(TabContentsWrapper* tab_contents,
  ...
  if (template_url) {
    ...
    // Carry over the user agent override.
...
void InstantLoader::CreatePreviewContents(TabContentsWrapper* tab_contents) {
 ...
 #if defined(OS_ANDROID)
   if (delegate_)
     delegate_->TabContentsCreated(preview_contents_.get());
 #endif
