...
class ExtensionInfoMap : public base::RefCountedThreadSafe<ExtensionInfoMap> {
 ...
 private:
  ...
  // The Android toolchain has trouble with forward declared structs
  // in std::pairs (used by std::map).  However, Clank does not
  // include any browser/extensions/ files, so the class must be both
  // declared (not forward declared) and defined in the header.
#if defined(OS_ANDROID)
  struct ExtraData {
    ExtraData() { CHECK(0); /* should NOT be used on Android */ }
  };
#endif
