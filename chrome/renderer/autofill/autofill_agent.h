...
class AutofillAgent : public content::RenderViewObserver,
 ...
 public:
  ...
  // See comments for ShowSuggestions() for a description of "caret at
  // end".  Set the variable which controls whether caret_at_end also
  // requires there be no selection.
  void SetRequiresNoSelectionForCaretAtEndCondition(bool requires) {
    requires_no_selection_for_caret_at_end_condition_ = requires;
  }
 ...
 private:
  ...
  // When requiring a "caret at end" condition (e.g. for autofill
  // matching), should we also require there is no selection?
  // In most cases, the default of true is fine.
  bool requires_no_selection_for_caret_at_end_condition_;
