...
AutofillAgent::AutofillAgent(
    ...
    : content::RenderViewObserver(render_view),
      ...
      requires_no_selection_for_caret_at_end_condition_(true),
...
void AutofillAgent::didAcceptAutofillSuggestion(const WebNode& node,
  ...
  if (suggestions_options_index_ != -1 &&
    ...
    // Clear the current composition, if there is one.
...
#if defined(OS_ANDROID)
  render_view()->GetWebView()->selectAutofillSuggestionAtIndex(listIndex);
#endif
}
...
void AutofillAgent::ShowSuggestions(const WebInputElement& element,
  ...
  if (requires_caret_at_end &&
      ((requires_no_selection_for_caret_at_end_condition_ &&
