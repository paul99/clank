...
void WriteToDiskTask(const FilePath& path, const std::string& data) {
  ...
  // Don't fsync on Android because it's very bad for performance.
  // All our supported devices use ext4 and it should be safe to skip the flush
  // on that file system for this particular use case. http://b/issue?id=6480318
#else
