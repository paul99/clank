...
base::StringPiece ResourceBundle::GetRawDataResource(int resource_id) const {
  ...
  if (!resources_data_->GetStringPiece(resource_id, &data)) {
    if (!locale_resources_data_->GetStringPiece(resource_id, &data)) {
      ...
      LOG(WARNING) << "Unable to load data resource with id " << resource_id;
      LOG(WARNING) << "Check if resource is in the white list. See " <<
          "clank/native/framework/resource_filter/README for details";
#endif
