...
#if defined(OS_ANDROID)
      LOG(WARNING) << "MAJOR PROBLEM IF YOU ARE ON CLANK";
#endif
      return string16();
    }
  }
...
#if defined(OS_ANDROID)
  LOG(WARNING) << "Check the corresponding grd and build files to make " <<
      "sure we include the resource for android.";
#endif
...
#if defined(OS_ANDROID)
  if (!bytes) {
    LOG(WARNING) << "Unable to load data bytes with id " << resource_id;
    LOG(WARNING) << "Check the corresponding grd and build files to make " <<
        "sure we include the resource for android.";
  }
#endif
