...
#if defined(OS_ANDROID)
    const std::string private_dirty_prefix = "Private_Dirty";
#endif
      ...
      switch (state) {
        ...
        case KEY_VALUE:
          ...
          if (last_key_name.starts_with(private_prefix)) {
            ...
            private_kb += cur;
#if defined(OS_ANDROID)
            if (last_key_name.starts_with(private_dirty_prefix))
              private_dirty_kb += cur;
#endif
...
#if defined(OS_ANDROID)
  ws_usage->priv_dirty = private_dirty_kb;
#endif
  return true;
}
