...
# since gyp_chromium is automatically forcing its inclusion.
{
  ...
  'variables': {
    ...
    # Set to 1 to enable profiling for code layout.  The flag changes the build
    # to include CFLAG -finstrument-functions to instrument all functions enter
    # and exit to call __cyg_profile_func_enter and __cyg_profile_func_exit.
    # The flag also will link to an implementation of the mentioned
    # instrumentation code, which is developed for profiling for code layout.
    # Profiling begins in the native code with a call to
    # cygprofile_start(filename) and ends at cygprofile_end().  This is used to
    # profile startup for code layout.  Currently only for Android target.
    'order_profiling%': 0,
    ...
    # No DCHECKs/assertions on release builds by default.
    ...
    'conditions': [
      ...
      # isn't supported (ARM)
      ['target_arch=="arm" and (chromeos == 1 or OS=="android")', {
        'disable_nacl%': 1,
       }, {
        'disable_nacl%': 0,
      }],
      ...
      ['order_profiling!=0', {
        'target_conditions' : [
          ['_toolset=="target"', {
            'conditions': [
              ['OS=="android"', {
                # -finstrument-functions causes new warnings treated as errors.
                # The instrumentation causes the compiler to raise warnings that
                # would not be caused otherwise, by confusing the compiler.
                # e.g. net/base/net_util.cc:529: error: 'lang_set' may be used
                # uninitialized in this function.  Without instrumentation, the
                # compiler is able to determine that lang_set will always be
                # initialized upon use, but can't determine it with
                # instrumentation even though the logic is the same. Thus, we
                # do not treat warnings as errors when compiling for profiling.
                'cflags': [ '-finstrument-functions', '-Wno-error' ],
              }],
            ],
          }], # condition for __toolset=="target"
        ],
      }],  # order_profiling!=0
      ['OS=="win"', {
        'defines': [
        ...
        'conditions': [
          ...
          [ 'os_posix==1 and os_bsd!=1 and OS!="mac" and OS!="android"', {
            'cflags': [
              ...
              # Also disabled on android, so we can build android targets on mac
              # which has an out-of-date GCC.
              '-Wno-unused-result',
            ],
          }],
          [ 'OS=="win"', {
    ...
    'configurations': {
      ...
      'Release_Base': {
        ...
        'conditions': [
          ...
          ['release_assertions==1', {
            # Android needs the option to enable DCHECK()s on release builds.
            'defines!': ['NDEBUG'],
          }],
        ],
      },
