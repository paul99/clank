...
class SingleTestRunner(BaseTestRunner):
  ...
  def __init__(self, device, test_suite, gtest_filter, test_arguments, timeout,
    ...
    # Below if condition was intentionally removed from upstream as Java based
    # unit tests hasn't been ready yet.
    if os.path.splitext(test_suite)[1] == '.apk':
      ...
      # The symbols_dir was intentionally removed from upstream as
      # ANDROID_PRODUCT_OUT is not available in upstream and we don't want to
      # expose that we are working on Android source tree.
      ...
      self.test_package = TestPackageExecutable(self.adb, device,
          test_suite, timeout, rebaseline, performance_test, cleanup_test_files,
          tool, self.dump_debug_info, symbols_dir)

  def _GetHttpServerDocumentRootForTestSuite(self):
    """Returns the document root needed by the test suite."""
    if self.test_package.test_suite_basename == 'performance_ui_tests':
  ...
  def GetDataFilesForTestSuite(self):
    ...
    if self.test_package.test_suite_basename in ['base_unittests',
      ...
      if self.test_package.test_suite_basename == 'unit_tests':
        ...
        old_cwd = os.getcwd()
        os.chdir(run_tests_helper.CHROME_DIR)
    ...
    elif self.test_package.test_suite_basename == 'performance_ui_tests':
      data = [
          'tools/page_cycler',
          'data/page_cycler',
          ]
  ...
  def MaybeRunLegacyBrowserBenchmarkBaseline(self):
    """Runs certain perf tests on the legacy browser for comparison."""
    if 'PageCycler' in self._gtest_filter:
      if 'MozHttp' in self._gtest_filter:
        legacy_browser_run_perftest.RunHttpPageCyclerBenchmark(
            self.device, 'moz')
      elif 'BloatHttp' in self._gtest_filter:
        legacy_browser_run_perftest.RunHttpPageCyclerBenchmark(
            self.device, 'bloat')
  ...
  def RunTestsWithFilter(self):
    ...
    self.MaybeRunLegacyBrowserBenchmarkBaseline()
  ...
  def SetUp(self):
    ...
    self.adb.ClearApplicationState(run_tests_helper.CHROME_PACKAGE)
