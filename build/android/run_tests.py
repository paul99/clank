...
def RunTests(device, test_suite, gtest_filter, test_arguments, rebaseline,
  ...
  if test_suite:
    # TODO(bulach): see TestPackageApk.StripAndCopyExecutable: we're not yet
    # building the APK directly. Remove the second condition.
    if (not os.path.exists(test_suite) and
        not os.path.splitext(test_suite)[1] == '.apk'):
      logging.critical('Test suite not found: %s' % test_suite)
...
def _RunATestSuite(options):
  ...
  if not run_tests_helper.ResetTestServerPortAllocation():
