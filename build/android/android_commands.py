...
class AndroidCommands(object):
  ...
  def StartActivity(self, package, activity, wait_for_completion=False,
    ...
    Args:
      ...
      activity: Name of activity (e.g. '.Main' or 'com.google.android.apps.chrome.Main').
  ...
  def CloseApplication(self, package):
    ...
    Args:
      package: Name of the process to kill off, e.g. com.google.android.apps.chrome
    """
  ...
  def IsAccountRegistered(self, account):
    """Returns True iff the given account name is registered on the device.

    This just looks to see if the account is mentioned in accounts.xml. It
    ignores whether the file lists the account as enabled or syncable.
    Perhaps there is a more canonical way of making this determination.
    """
    accounts_xml = self.GetFileContents('/data/system/sync/accounts.xml')
    for line in accounts_xml:
      if account in line:
        return True
    return False
  ...
  def StartRecordingLogcat(self, clear=True, filters=['*:v']):
    ...
    logcat_command = 'adb %s logcat -v threadtime %s' % (self._adb._target_arg, ' '.join(filters))
  ...
  def GetSurfaceStats(self):
    """Returns the current Surface index and timestamp.
    ...
    """
    results = self.RunShellCommand('service call SurfaceFlinger 1013')
  ...
  def GetMemoryUsageForPid(self, pid):
    ...
    for line in self.GetFileContents('/proc/%s/smaps' % pid, log_result=False):
      ...
      if match:
        usage_dict[match.group('key')] += int(match.group('usage_kb'))
    ...
    # On the tablet, graphics driver allocations are not included in smaps. So
    # we must grab it from nvmap.
    # TODO(tonyg): Remove this when http://b/5773193 is fixed.
  ...
  def GetMemoryUsage(self, package):
    ...
    for pid in pid_list:
      usage_dict_per_pid = self.GetMemoryUsageForPid(pid)
  ...
  def StayAwake(self):
    """Makes sure the screen does not sleep."""
    self._adb.SendShellCommand('svc power stayon true')
  ...
  def SetSystemSetting(self, table, key, value):
    """Sets a system setting on the device.

    System settings are stored in sqlite on the device in:
      /data/data/com.android.providers.settings/databases/settings.db

    A comprehensive list of settings is not documented, so the best way to find
    the table/key/value for a setting is to explore the db manually, e.g.
      $ adb shell
      # sqlite3 /data/data/com.android.providers.settings/databases/settings.db
      sqlite> .tables
      sqlite> select * from system;

    TODO(tonyg): JB supports a new, less brittle "adb shell content" API for
    these settings. Switch to that as soon as we can.

    Args:
      table: The table where the key for the setting is stored.
      key: The string key of the setting.
      value: The new value for the the setting.

    Raises:
      Exception if the setting was not properly set.
    """
    db = _SYSTEM_SETTINGS_PATH
    self._SetOrUpdateSettingsDatabaseRow(db, table, ['name', 'value'],
                                         [key, value])

  def HasLockScreenSettings(self):
    """Checks whether the device has a dedicated database for lock screen
    settings.

    Returns:
      True if lock screen settings are available, False otherwise.
    """
    return self.FileExistsOnDevice(_LOCK_SCREEN_SETTINGS_PATH)

  def SetLockScreenSetting(self, table, key, value, user = 0):
    """Sets a lock screen setting on the device.
    ...
    Args:
      ...
      key: The string key of the setting.
      value: The new value for the the setting.
      user: User ID, 0 by default which is the default user.

    Raises:
      Exception if the setting was not properly set.
    """
    db = _LOCK_SCREEN_SETTINGS_PATH
    self._SetOrUpdateSettingsDatabaseRow(db, table,
                                         ['name', 'user', 'value'],
                                         [key, str(user), value])

  def _SetOrUpdateSettingsDatabaseRow(self, db, table, columns, values):
    """Sets a generic database setting on the device.

    IMPORTANT: The first column is used as a primary key so that all rows with
    the same value for that column are removed from the table prior to
    inserting the new values.

    Args:
      db: On-device path of the sqlite database file to be modified.
      table: The table where the key for the setting is stored.
      columns: List of columns to set or update.
...
commit transaction;""" % {
    ...
    error_msg = self._adb.SendShellCommand('sqlite3 %s "%s"' % (db, cmd))
    if error_msg:
...
def main(argv):
  # TODO(tonyg/joth): This file should not be executable. Remove this method.
  # Do not add new params.
