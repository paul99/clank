...
class TestPackage(object):
  ...
  def __init__(self, adb, device, test_suite, timeout, rebaseline,
    ...
    if timeout == 0:
      if self.test_suite_basename == 'performance_ui_tests':
        timeout = 900
      else:
        timeout = 60
  ...
  def GetDisabledPrefixes(self):
    # TODO(joth): Remove FAILS_ from this list, as those tests should still
    # deterministically run to completion (but see http://b/5562913)
  ...
  def _WatchTestOutput(self, p):
    ...
    # Early quit if encountering problem when launching chromr for page cycler.
    self.adb.StartMonitoringLogcat(clear=True, timeout=120)
    if self.test_suite_basename == 'performance_ui_tests':
      try:
        ...
        self.adb.WaitForLogMatch(chrome_launch_done_re, None)
